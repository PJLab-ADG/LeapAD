...
class ScenarioManager(object):
    ...
    def __init__(self, timeout, debug_mode=False):
        ...
        watchdog_timeout = max(100, self._timeout - 2)
    ...
    def signal_handler(self, signum, frame):
        ...
        if signum == signal.SIGINT:  
            print("Received SIGINT signal (Ctrl+C pressed)")  
        elif signum == signal.SIGTERM:  
            print("Received SIGTERM signal")  
        elif signum == signal.SIGKILL:  
            print("Received SIGKILL signal (forceful termination)")  
        self._running = False
    ...
    def _tick_scenario(self, timestamp):
        ...
        if self._timestamp_last_run < timestamp.elapsed_seconds and self._running:
            ...
            try:
                    ...
                    if node.list_traffic_events:
                        ...
                        for event in node.list_traffic_events:
                            if event.get_type() == TrafficEventType.COLLISION_STATIC:
                                event_messages.append(event.get_message())
                ...
                ego_action = self._agent(world, event_messages)
